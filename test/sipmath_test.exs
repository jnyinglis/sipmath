defmodule SIPmathTest do
  ...
  alias SIPmath
  import SIPmath.Distribution.{Cycle, Normal, Sequence, Uniform}
  ...
  test "#2 - Uniform HDR: confirm random numbers between 1 and 6" do
    with min = 1,
         max = 6,
         seed_value = 1,
         uniform_dist = uniform(seed_value, min, max, "x")
    do
  ...
  test "#4 - Normal HDR: confirm first random number" do
    with  seed_value = 2,
          mean = 4,
          std_dev = 12,
          normal_dist = normal(seed_value, mean, std_dev, "x")
    do
...
# end
  ...
  test "#6 - SIPmath.cycle: Test that cycle returns the given values as a stream, and repeats when the requested output is longer than the input" do
    historical_demand = [5, 6, 10, 8, 7, 10, 5, 5, 3, 2, 6, 5, 6, 3, 5, 4, 5, 4, 4, 3, 3, 3, 4, 3, 8, 3, 5, 2, 4, 5, 7, 6, 7, 8, 1, 5]

    with  cycle_dist = cycle(historical_demand, "x")
    do
  ...
  test "#7 - SIPmath.cycle: Test Beyond Budget" do
    ...
    with  demand_dist = cycle(historical_demand, "demand"),
          amt_stocked_dist = cycle([5], "stocked"),
          expiration_dist = cycle([50], "expiration"),
          airfreight_dist = cycle([150], "airfreight"),
  ...
  test "#9 - SIPmath.sequence/2" do
    with  start_value = 1,
          step_value = 1,
          sequence_dist = sequence(start_value, step_value,"sequence")
    do
