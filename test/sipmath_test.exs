...
# end
  ...
  test "SIPmath.cycle/3 Test that cycle returns the given values as a stream, and repeats when the requested output is longer than the input" do
    historical_demand = [5, 6, 10, 8, 7, 10, 5, 5, 3, 2, 6, 5, 6, 3, 5, 4, 5, 4, 4, 3, 3, 3, 4, 3, 8, 3, 5, 2, 4, 5, 7, 6, 7, 8, 1, 5]

    with  sv_id = 1,
          cycle_dist = SIPmath.cycle("x", sv_id, historical_demand)
    do
  ...
  test "SIPmath.cycle/3 Test Beyond Budget" do
    historical_demand = [5, 6, 10, 8, 7, 10, 5, 5, 3, 2, 6, 5, 6, 3, 5, 4, 5, 4, 4, 3, 3, 3, 4, 3, 8, 3, 5, 2, 4, 5, 7, 6, 7, 8, 1, 5]
    ...
    with  sv_id = 1,
          demand_dist = SIPmath.cycle("demand", sv_id, historical_demand),
          amt_stocked_dist = SIPmath.cycle("stocked", sv_id, [5]),
          expiration_dist = SIPmath.cycle("expiration", sv_id, [50]),
          airfreight_dist = SIPmath.cycle("airfreight", sv_id, [150]),
  ...
  test "SIPmath.cycle/3 Fail when an empty list or no values are supplied" do
    empty_list = []
    sv_id = 1

    assert_raise FunctionClauseError, fn ->
      SIPmath.cycle("x", sv_id, empty_list)
    end

    assert_raise FunctionClauseError, fn ->
      SIPmath.cycle("x", sv_id, nil)
    end
  end
