defmodule SIPmath.Distribution.Uniform do
  ...
  @spec uniform(seed_value :: pos_integer(), min :: number(), max :: number(), name :: String.t()) ::
          State.t()
  ...
  defimpl SIPable do
    ...
    @spec next_value(type_specific_state :: Uniform.t(), pm_index :: pos_integer()) ::
            {number(), Uniform.t()}
    def next_value(type_specific_state = %Uniform{}, pm_index) do
      with %{seed_value: seed_value, min: min, max: max} = type_specific_state do
