defmodule SIPmath.Distribution.Normal do
  ...
  def new(seed_value, mean, std_dev)
      when is_integer(seed_value) and is_number(mean) and is_number(std_dev) do
    %__MODULE__{seed_value: seed_value, mean: mean, std_dev: std_dev}
  end

  @spec normal(seed_value :: integer(), mean :: number(), std_dev :: number(), name :: String.t()) ::
          State.t()
  ...
  defimpl SIPable do
    ...
    @doc """
    """
    @spec next_value(type_specific_state :: Normal.t(), pm_index :: integer()) ::
            {number(), Normal.t()}
    def next_value(type_specific_state = %Normal{}, pm_index) do
      with %{seed_value: seed_value, mean: mean, std_dev: std_dev} = type_specific_state do
