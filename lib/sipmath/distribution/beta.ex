defmodule SIPmath.Distribution.Beta do
  ...
  def new(seed_value, alpha, beta, a, b)
      when is_integer(seed_value) and is_number(alpha) and is_number(beta) do
  ...
  defimpl SIPable do
    ...
    @doc """
    """
    @spec next_value(type_specific_state :: Beta.t(), pm_index :: integer()) ::
            {number(), Beta.t()}
    def next_value(type_specific_state = %Beta{}, pm_index) do
      with %{seed_value: seed_value, alpha: alpha, beta: beta, a: a, b: b} = type_specific_state do
